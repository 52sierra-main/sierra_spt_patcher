...
class RepositorySierraPatcherGUI(HybridSierraPatcherGUI):
    ...
    def __init__(self, dev: bool = False):
        ...
        if dev:
            ...
            if notebook is not None:
                ...
                notebook.insert(1, self._repository_tab, text="Repository")
    ...
    def _build_repository_tab(self, nb) -> ttk.Frame:
        ...
        selector = ttk.LabelFrame(root, text="Local repository")
        ...
        self.r_release_var = tk.StringVar(value=_REPOSITORY_RELEASE_PLACEHOLDER)

        ttk.Label(selector, text="Repository directory").grid(
        ...
        ttk.Button(selector, text="Browse...", command=self._repository_browse).grid(
            row=0, column=2, padx=(8, 10), pady=(8, 4)
        )

        ttk.Label(selector, text="Release").grid(
        ...
        self.r_release = ttk.Combobox(
            ...
            state="readonly",
            values=(_REPOSITORY_RELEASE_PLACEHOLDER,),
        )
        ...
        ttk.Button(selector, text="Refresh", command=self._repository_refresh).grid(
            row=1, column=2, padx=(8, 10), pady=(4, 8)
        )

        metadata = ttk.LabelFrame(root, text="Release metadata")
        ...
        ttk.Label(metadata, text="Integrity folders").grid(
        ...
        ttk.Label(
            metadata,
            text="JSON object. Clearing this removes aggregate folder-size checks from the release metadata.",
            foreground="#666",
            wraplength=330,
        ...
        self.r_btn_catalog = ttk.Button(
            maintenance,
            text="Rebuild catalog.json from local releases",
        ...
        self.r_status_var = tk.StringVar(value="Select a repository release.")
        ttk.Label(
            root,
    ...
    def _repository_selected_release(self) -> str | None:
        ...
        if not value or value == _REPOSITORY_RELEASE_PLACEHOLDER:
            return None
        return value

    def _repository_browse(self) -> None:
        selected = filedialog.askdirectory(title="Select Sierra web repository")
        if selected:
    ...
    def _repository_refresh(self) -> None:
        ...
        try:
            ...
            releases = []
            self.r_status_var.set(f"Could not inspect repository: {exc}")
        values = (_REPOSITORY_RELEASE_PLACEHOLDER, *releases)
        ...
        if current in releases:
            ...
            self.r_release_var.set(_REPOSITORY_RELEASE_PLACEHOLDER)
            self._repository_metadata_original = None
            self._repository_clear_editor()
            self._repository_set_editor_enabled(False)
            if releases:
                self.r_status_var.set(f"Local repository contains {len(releases)} release(s).")
            else:
                self.r_status_var.set("No release manifests found in this local repository.")
    ...
    def _repository_load_metadata(self) -> None:
        ...
        try:
            ...
            self._repository_clear_editor()
            self._repository_set_editor_enabled(False)
            self.r_status_var.set(f"Could not load {release} metadata: {exc}")
            return
        ...
        self.r_status_var.set(f"Loaded metadata for {release}.")

    def _repository_clear_integrity(self) -> None:
        self.r_integrity_text.configure(state="normal")
        self.r_integrity_text.delete("1.0", tk.END)
        self.r_integrity_text.insert("1.0", "{}")
        self.r_status_var.set("Integrity checks cleared in the editor. Click Save metadata to publish the change locally.")

    def _repository_collect_metadata(self) -> dict:
        if self._repository_metadata_original is None:
            raise RepositoryToolError("no release metadata is loaded")
        try:
            integrity = json.loads(self.r_integrity_text.get("1.0", tk.END).strip() or "{}")
        except json.JSONDecodeError as exc:
            raise RepositoryToolError(f"integrity folders JSON is invalid: {exc}") from exc
        if not isinstance(integrity, dict):
            raise RepositoryToolError("integrity folders must be a JSON object")
        for path, size in integrity.items():
            if not isinstance(path, str) or not path.strip():
                raise RepositoryToolError("integrity folder paths must be non-empty strings")
            if not isinstance(size, int) or size < 0:
                raise RepositoryToolError(f"integrity size for {path!r} must be a non-negative integer")
    ...
    def _repository_save_metadata(self) -> None:
        release = self._repository_selected_release()
        if not release:
            messagebox.showerror("Release required", "Choose a local repository release first.")
            return
        try:
        ...
        if not messagebox.askyesno(
            "Update local release metadata",
            f"Update metadata for {release} in the local repository?\n\n"
        ...
        try:
            ...
            messagebox.showerror("Repository metadata", str(exc))
            return

        self._repository_metadata_original = dict(data)
        self.r_status_var.set(
            f"Updated {release} metadata. New object: {object_id}. "
    ...
    def _repository_rebuild_catalog(self) -> None:
        try:
            ...
            messagebox.showerror("Repository catalog", str(exc))
            return
        self.r_status_var.set(
            f"Rebuilt {catalog_path.name} with {len(releases)} local release(s): "
    ...
    def _repository_verify(self) -> None:
        ...
        self.r_status_var.set(f"Verifying {release}...")
        ...
        def progress(current: int, total: int, logical_path: str) -> None:
            _safe_call(
                self,
                self.r_status_var.set,
                f"Verifying {release}: {current}/{total}  {logical_path}",
            )
        ...
        def worker() -> None:
            try:
                ...
                message = (
                    f"Verified {release}: {result.file_count} logical file(s), "
                    f"{result.object_references} object reference(s), {mib:,.1f} MiB."
                )
                ...
                _safe_call(self, self.r_status_var.set, f"Verification failed: {exc}")
                _safe_call(self, messagebox.showerror, "Repository verification", str(exc))
            finally:
                _safe_call(self, self.r_btn_verify.configure, state="normal")
